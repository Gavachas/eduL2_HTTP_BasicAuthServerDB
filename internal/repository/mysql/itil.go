	...
	if err = m.db.Ping(); err != nil {
		if dbtype == "mysql" {
			...
			if err != nil {
				...
				m.db, err = sql.Open(dbtype, "root:159753@tcp(docker.for.mac.localhost:3306)/")
				if err != nil {
...
func (i *ItilModel) initBaseMain() error {
	//err := r.drv.InitBaseMain()
	var err error
	_, err = i.db.Exec("CREATE DATABASE IF NOT EXISTS Itil")
	if err != nil {
		return err
	}
	log.Println("CREATE  Itil", err)
	_, err = i.db.Exec("USE Itil")
	if err != nil {

		return err
	}
	log.Println("insert users  ", err)
	_, err = i.db.Exec(`DROP TABLE IF EXISTS incidents;`)
	if err != nil {

		return err
	}

	_, err = i.db.Exec(`CREATE TABLE IF NOT EXISTS incidents (
	...
	_, err = i.db.Exec(`DROP TABLE IF EXISTS rules;`)

	if err != nil {

		return err
	}
	_, err = i.db.Exec(`CREATE TABLE IF NOT EXISTS rules (
	  id int NOT NULL AUTO_INCREMENT,
	  name varchar(200) COLLATE utf8mb4_unicode_ci NOT NULL,
	  userID int NOT NULL,
	  PRIMARY KEY (id)
	...
	_, err = i.db.Exec(`DROP TABLE IF EXISTS users;`)
	if err != nil {

		return err
	}
	_, err = i.db.Exec(`CREATE TABLE IF NOT EXISTS users (
	  id int NOT NULL AUTO_INCREMENT,
